...
describe 'Test Collaborator Handling' do
  ...
  describe 'Adding collaborators into giftlist' do
    it 'HAPPY: should add a valid collaborator' do
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      put "api/v1/giftlists/#{@proj.id}/collaborators", req_data.to_json
  ...
  describe 'Removing collaborators from a giftlist' do
    it 'HAPPY: should remove with proper authorization' do
      @proj.add_collaborator(@another_account)
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      delete "api/v1/giftlists/#{@proj.id}/collaborators", req_data.to_json
